...
try:
    ...
    print("Error acquiring marketcap data")
...
price_list = [0] * len(coin_name)

for ticker in coin_ticker:
    try:
        price_list[coin_ticker.index(ticker)] = coinmarketcap.ticker(ticker.upper())['price_usd']
    except:
        print("No price for ticker: " + ticker)
...
csv_prices = pd.read_csv("Coin Prices.csv")
csv_prices[str(now)] = price_list
csv_prices.to_csv("Coin Prices.csv", index=False)
